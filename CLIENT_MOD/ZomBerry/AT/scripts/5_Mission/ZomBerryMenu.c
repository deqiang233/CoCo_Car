class ZomberryMenu extends UIScriptedMenu {
	...
	protected string m_objTarget = "光标";
	...
	override void OnShow() {
		...
		m_TxtTitle.SetText( "在线玩家: ... | 免费公益 " + g_zbryVer + " 汉化联系：413198961" );
		...
		if ( m_objTarget != "脚下" ) m_SpawnTargetButton.SetText( m_objTarget );
	}
	...
	override bool OnClick( Widget w, int x, int y, int button ) {
		...
		if ( w == m_SpawnButton ) {
			...
			if (!m_spawnModeExpert) {
				...
				m_MultiButton.SetText( "安全" );
			} else {
				m_MultiButton.SetTextColor(COLOR_RED);
				m_MultiButton.SetText( "专家" );
			}
		}
		...
		if ( w == m_MultiButton ) {
			if (m_FunctionsPage.IsVisible()) {
				...
				if (selFunc == -1) { Message("没有选择功能. [双击] 删除绑定, [A] + [双击] 删除所有绑定."); return true; }

				m_FunctionsList.GetItemData( selFunc, 0, funcData );
				if (funcData.param3 == -1) return true;
				if (m_funcParams[funcData.param3].Count() > 0) { Message("此函数需要执行参数，因此键绑定被禁用（将在未来可用）"); return true; }
				int oldKey = ZomberryBase.GetKeyBindsMgr().GetFuncKey(funcData.param1);

				if (m_setBindMode != -1 && (KeyState(KeyCode.KC_A) & 0x00000001)) {
					m_MultiButton.SetText("清除?");
					m_setBindMode = -1;
	...
	override bool OnDoubleClick( Widget w, int x, int y, int button ) {
		...
		if ( w == m_PlayersList ) {
			...
			m_PlyHealth.SetText("  健康: " + plyData.param3.ToString());
			m_PlyBlood.SetText("  血量: " + plyData.param4.ToString());
		}
		...
		if ( w == m_FunctionsList ) {
			...
			if ( funcData.param2 ) {
				if ( FindPlyInList(m_lastSelPlayer) == -1 ) { Message("没有选择玩家"); return true; }
		...
		if ( w == m_ObjectsList ) {
			...
			if (currentObjectData == "Entry") {
				...
				if (m_objTarget == "脚下") targetPlace = adminPly.GetPosition();
				if (m_objTarget == "库存") targetInventory = true;
	...
	static void Message( string txt ) {
		GetGame().GetMission().OnEvent(ChatMessageEventTypeID, new ChatMessageEventParams(CCAdmin, "[免费公益]", txt, ""));
	}
	...
	void SyncPlayers( CallType type, ref ParamsReadContext ctx, ref PlayerIdentity sender, ref Object target ) {
		...
		if (layoutRoot.IsVisible()) {
			...
			m_TxtTitle.SetText( "在线玩家：" + playerListC.Count() + " | " + Math.Floor(sUpTime / 3600).ToString() + "h " + ((sUpTime / 60) % 60).ToString() + "分 | 免费公益 " + g_zbryVer + " 汉化联系：413198961");
