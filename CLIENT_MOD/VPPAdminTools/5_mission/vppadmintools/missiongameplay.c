...
modded class MissionGameplay
{
    ...
    */
    ...
    void ExitSpectate()
    {
        ...
        {
            // Reset spectate mode first
            g_Game.SetSpectateMode(false);
            
            // Find and delete the spectate camera - use Object as intermediate type
            Object playerObj = GetGame().GetPlayer();
            VPPSpectateCam cam = VPPSpectateCam.Cast(playerObj);
            if (cam)
            {
                // Clear target before deleting
                cam.SetTargetObj(null);
            ...
            // Reconnect to restore player entity
