class PlayerManager extends PluginBase
{
	...
	void ChangePlayerScale(CallType type, ParamsReadContext ctx, PlayerIdentity sender, Object target)
	{
		...
		foreach(string targetID : ids)
		{
			if (GetPermissionManager().VerifyPermission(sender.GetPlainId(),"PlayerManager:ChangeScale",targetID))
			{
				...
				if(targetPlayer)
				{
					GetPermissionManager().NotifyPlayer(sender.GetPlainId(), string.Format("#VSTR_NOTIFY_SCALE_CHANGED", targetPlayer.VPlayerGetName(), data.param2.ToString()), NotifyTypes.NOTIFY);
	...
	void MakePlayerVomit(CallType type, ParamsReadContext ctx, PlayerIdentity sender, Object target)
	{
		...
		foreach(string targetID : ids)
		{
			if (GetPermissionManager().VerifyPermission(sender.GetPlainId(),"PlayerManager:MakePlayerVomit",targetID))
			{
				...
				{
					GetPermissionManager().NotifyPlayer(sender.GetPlainId(), string.Format("#VSTR_NOTIFY_VOMIT", targetPlayer.GetName(), data.param2.ToString()), NotifyTypes.NOTIFY);
	...
	void SpectatePlayer(CallType type, ParamsReadContext ctx, PlayerIdentity sender, Object target)
	{
		if (type == CallType.Server)
        {
			...
			if (GetPermissionManager().VerifyPermission(sender.GetPlainId(),"PlayerManager:SpectatePlayer",data.param1))
			{
				...
				{
					GetPermissionManager().NotifyPlayer(sender.GetPlainId(),"#VSTR_NOTIFY_SPECTATE_SELF",NotifyTypes.ERROR);
					return;
				}
				...
				if (tgp != null)
				{
					...
					// Move admin player to below map if exists (first time spectating)
					if (adminPlayer != null)
						adminPlayer.SetPosition(Vector(tgp.GetPosition()[0],-120,tgp.GetPosition()[2]));
					// If admin is already in spectate mode, InvokeSpectate will handle the camera directly

					// Use shorter delay if already in spectate mode (continuous spectating)
					float delay;
					if (adminPlayer != null)
						delay = 2.0;
					else
					...
					m_RPCDelay.Run(delay,this,"InvokeSpectate", new Param2<string,string>(data.param1,sender.GetPlainId()),false);
	...
	void InvokeSpectate(string id, string adminID)
	{
		...
		if (adminPlayer != null) {
			...
			// Delete admin player entity if exists
			GetGame().ObjectDelete(adminPlayer);
		} else {
			// Admin is already in spectate mode (player entity deleted), find identity from all players
			array<Man> players = new array<Man>;
			GetGame().GetPlayers(players);
			foreach (Man player : players) {
				if (player && player.GetIdentity() && player.GetIdentity().GetPlainId() == adminID) {
		...
		// Send RPC to initialize spectate camera
		if (adminIdentity) {
			GetRPCManager().VSendRPC( "RPC_MenuPlayerManager", "InitSpectate", new Param1<Object>(spectateTarget), true, adminIdentity );
		}
	}
				...
				if (GetPermissionManager().VerifyPermission(adminID, "PlayerManager:FreezePlayers", id))
				{
					...
					if (targetPlayer != NULL)
					{
						...
						if ( targetPlayer.VPPIsFreezeControls() )
							GetPermissionManager().NotifyPlayer(sender.GetPlainId(), string.Format("#VSTR_NOTIFY_FREEZE_ON", ids.Count().ToString()), NotifyTypes.NOTIFY);
						else
							GetPermissionManager().NotifyPlayer(sender.GetPlainId(), string.Format("#VSTR_NOTIFY_FREEZE_OFF", ids.Count().ToString()), NotifyTypes.NOTIFY);
			...
			foreach(int id : ids)
			{
				...
				if (pb != NULL)
				{
					...
					if (pb.InvisibilityStatus())
						GetPermissionManager().NotifyPlayer(sender.GetPlainId(), string.Format("#VSTR_NOTIFY_INVIS_ON", ids.Count().ToString()), NotifyTypes.NOTIFY);
					else
						GetPermissionManager().NotifyPlayer(sender.GetPlainId(), string.Format("#VSTR_NOTIFY_INVIS_OFF", ids.Count().ToString()), NotifyTypes.NOTIFY);
				}
			}
