class MenuPlayerManager extends AdminHudSubMenu
{
	...
	override void OnCreate(Widget RootW)
	{
		...
		GetVPPUIManager().HookConfirmationDialog(m_ActionScalePlayer, M_SUB_WIDGET, this, "PlayerScaleDiag", DIAGTYPE.DIAG_OK_CANCEL_INPUT, "#VSTR_DIALOG_TITLE_SCALE", "#VSTR_DIALOG_CONTENT_SCALE");
		//--------------
	...
	*/
	...
	void InitSpectate(CallType type, ParamsReadContext ctx, PlayerIdentity sender, Object target)
	{
		...
		if( type == CallType.Client )
		{
			if (data.param1 != null)
			{
				VPPSpectateCam cam = null;
				PlayerBase targetPlayer = PlayerBase.Cast(data.param1);
				if (!targetPlayer) return;
				
				// Get target position with eye height
				vector targetPos = targetPlayer.GetPosition();
				targetPos[1] = targetPos[1] + 1.6;
				
				// Check if spectate camera already exists (for continuous spectating)
				Object playerObj = GetGame().GetPlayer();
				if (playerObj != null)
				{
					cam = VPPSpectateCam.Cast(playerObj);
					if (cam)
					{
						// Reuse existing camera, just update target
						cam.SetTargetObj(targetPlayer);
						cam.SetPosition(targetPos);
						cam.SetOrientation(targetPlayer.GetOrientation());
						
						// Force update spectator position
						GetGame().UpdateSpectatorPosition(targetPos);
					}
					else
					{
						// Old player entity exists, clean it up completely
						GetGame().SelectPlayer(null, null);
						GetGame().ObjectDelete(playerObj);
						
						// Small delay to ensure cleanup
						GetGame().GetCallQueue(CALL_CATEGORY_SYSTEM).CallLater(this.CreateSpectateCamera, 100, false, targetPlayer, targetPos);
						return;
					}
				}
				else
				{
					// No existing player object, create camera immediately
					CreateSpectateCamera(targetPlayer, targetPos);
				}
				
				// Ensure spectate mode is active
				if (cam)
	...
	void CreateSpectateCamera(PlayerBase targetPlayer, vector targetPos)
	{
		if (!targetPlayer) return;
		
		// Create camera at target position
		VPPSpectateCam cam = VPPSpectateCam.Cast(GetGame().CreateObject("VPPSpectateCam", targetPos, true));
		if (cam)
		{
			cam.SetTargetObj(targetPlayer);
			cam.SetOrientation(targetPlayer.GetOrientation());
			GetGame().SelectPlayer(null, cam);
	...
	void SendMessageToPlayer(int result,string userInput)
	{
		...
		{
			array<string> uids = GetSelectedPlayersIDs();
        	GetRPCManager().VSendRPC( "RPC_PlayerManager", "SendMessage", new Param3<string,string,ref array<string>>("#VSTR_SERVER_ADMIN_PREFIX",userInput,uids), true );
		}
	}
